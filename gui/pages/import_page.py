...
class ImportPage(QWidget):
    ...
    def create_image_grid(self) -> QListWidget:
        ...
        #
        # 选中态的 color 必须显式写死：不写的话，画选中项的文字用的是调色板里的
        # HighlightedText——Fusion 下那是白的，而这里的格子底色仍然是浅色（我们
        # 没给选中项铺蓝底），白字落在白底上等于文件名整条消失。选中与否由蓝色
        # 边框表达，文字一律用正文色；图片也不蒙蓝（见 _thumbnail_icon）。
